...
class Net(nn.Module):
  def __init__(self):
    ...
    self.conv1_bn = nn.BatchNorm2d(8) # applies batch norm after conv layer 1
...
def train():	 
		...
		if (epoch+1) % 5 == 0:
			...
			with torch.no_grad():
				for iter_val, val_data in enumerate(val_loader):
					...
					#bacth size for val set is 100, so 100 preds appended
					for p in val_preds:
	...
	if not os.path.exists(LOG_DIR):
	    os.makedirs(LOG_DIR)
	torch.save(net.state_dict(), os.path.join(LOG_DIR,'checkpoint'+ str(epoch) +'.pt'))
	hw3utils.visualize_batch(val_inputs,val_preds,val_targets,os.path.join(LOG_DIR,'example' + str(epoch) + '.png'))	
